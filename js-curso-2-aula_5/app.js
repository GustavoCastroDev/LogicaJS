...
// Função que limpa o campo onde o usuário insere o número após tentativa
...
// Criando função que exibe mensagens inciais ao abrir o jogo
function exibeMensagemInicial() {
    exibirTextoNaTela('h1', 'Jogo do número secreto');
    exibirTextoNaTela('p', 'Escolha um número entre 1 e 10');
}

// Criando função que reinicia a página oa clicar em "Reiniciar"
function reiniciarJogo() {
    // Reutilizando a variável 'numeroSecreto' para gerar um novo número aleatório por meio da função 'gerarNumeroAleatorio'
    numeroSecreto = gerarNumeroAleatorio();
    // Reiniciando o valor da variável 'tentativa'
    tentativa = 1;
    // Chamando a função que limpa os campos
    limparCampo();
    // Chamando a função que reescreve as mensagens inicias da página
    exibeMensagemInicial();
    // Propriedade 'setAttribute' retorna o status do botão "Reiniciar" para 'disabled' por meio do ID 'reiniciar'
...
// Chamando a função que reinicia o jogo fora dos blocos de lógica
reiniciarJogo();
